...
(function () {
  ...
  /* ------------------------------------------------------------ fixtures */
  /* A construction plot: hatched floor, a fill bar for what you have paid
     in so far, and a card saying what it will be and what is left to pay.
     Used by every product line, every cafe machine and every cafe table. */
  ...
  function drawBubble(ctx, c, head) {
    /* In the mini mart the bubble is a shopping list, so it goes away once
       ...
       need to read — so it stays up until the drink is in their hand. */
    const cafe = MSM.cafe.active();
    if (cafe) {
  ...
  /* Painting kit for src/cafe-render.js — the cafe draws its own fixtures
     but there is no reason for it to reinvent a rounded rectangle. */
  ...
  R.frame = function (dt) {
    ...
    MSM.econ.store().products.forEach((prod, n) => {
      ...
      if (built) {
        ...
        if (prod.shelf && !cafe) {
          items.push({ d: prod.shelf.x1 + prod.shelf.y1, fn: () => drawShelf(ctx, n) });
        }
      }
    });
    ...
    MSM.ent.customers.forEach((c) => items.push({
      ...
      fn: () => {
        ...
        // a cafe customer holds a cup, not a shopping basket — MSM.cafe draws it
 ...
 + U.money(Math.max(0, cost - paid)), c.x, c.y - h * 0.28 - 6, h * 0.36,
         MSM.state.cash > 0 ? '#2CA85C' : '#98A6C4');
  }
